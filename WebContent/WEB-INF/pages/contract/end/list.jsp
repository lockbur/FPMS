...
function listEnd()
{
	...
	form.action="<%=request.getContextPath()%>/contract/end/list.do?<%=WebConsts.FUNC_ID_KEY%>=03020702";
	App.submit(form);
}
...
function preEdit(cntNum){
	var form=$("#endForm")[0];
	form.action="<%=request.getContextPath()%>/contract/end/view.do?<%=WebConsts.FUNC_ID_KEY%>=0302070202&cntNum="+cntNum;
	App.submit(form);
}
...
<body>
<p:authFunc funcArray="0302070202"/>
	...
	<table id="listTab" class="tableList">
		...
		<c:forEach items="${eList}" var="e" varStatus="status">
			<tr onmouseover="setTrBgClass(this, 'trOnOver2');" onmouseout="setTrBgClass(this, 'trOther');">				
				...
				<td >
					<div class="update">
					    <a href="#" onclick="preEdit('${e.cntNum}');" title="合同终止"></a>
					</div>
				</td>	
			</tr>
		</c:forEach>
