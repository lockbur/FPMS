...
function toFinish(cntNum){
	//终止
	var url = "<%=request.getContextPath()%>/contract/end/finish.do?<%=WebConsts.FUNC_ID_KEY %>=0302070201";
	$("#auditDiv").dialog({
		title:"合同完成",
		closeOnEscape:false,
		autoOpen: true,
		...
		buttons: {
			"确定": function() {
				...
				//对是否选择了一次预提待摊进行判断（合同为预提待摊类）
				var isPrepaidProvision="${cnt.isPrepaidProvision}";
				var prepaidStatus=$("#prepaidStatus").val();
				var feeEndDate = "${cnt.feeEndDate}";
				var today ="${nowDate}";
				if(isPrepaidProvision=='0' && feeEndDate >= today){
					if(prepaidStatus==''||prepaidStatus==null){
						App.notyError("请先选择是否一次待摊!");
						return false;
					}
				}
...
function toSelectResult(s){
	if(s==$("#prepaidStatus").val()){
		return false;
	}
	if($("#prepaidStatus").val()!=''&&$("#prepaidStatus").val()!=null){
		$("#waterMemo").val("");
	}
	if(s=='Y'){
		//生成
...
<%-- 										<td><fmt:formatNumber type="number" value="${dzspItem.abcdeAmt}"/></td> --%>
				...
				<tr class="collspan-control">
					...
					<tr>
						<td class="tdLeft">费用类型
							<%-- <img border='0' id='feeInfoImg' alt='费用信息' width='25px' src='<%=request.getContextPath()%>/common/images/search.jpg' onclick='queryFeeTypePage("${cnt.cntNum}")' align="center"/> --%>
						</td>
		...
		<tr>
			<td colspan="4" >
				...
				<input type="button" value="合同完成" onclick="toFinish('${cnt.cntNum}');"> 
