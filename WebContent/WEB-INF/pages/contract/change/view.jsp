...
function toChange(cntNum){
	var url = "<%=request.getContextPath()%>/contract/change/toChange.do?<%=WebConsts.FUNC_ID_KEY %>=0302040102";
	$("#auditDiv").dialog({
		title:"发起变更",
		closeOnEscape:false,
		autoOpen: true,
...
// 				if(($.trim($("#waterMemo").val())==null)||($.trim($("#waterMemo").val())=='')){
// 					App.notyError("变更申请备注不能为空！");
// 					return;
// 				}
...
<%-- 										<td><fmt:formatNumber type="number" value="${dzspItem.abcdeAmt}"/></td> --%>
			...
			<td>
				<table class="tableList">
					...
					<tr>
						...
						<td class="tdRight">${cnt.houseKindIdName}</td>
		...
		<tr>
			<td colspan="4" >
				...
				<input type="button" value="发起变更" onclick="toChange('${cnt.cntNum}');"> 
