...
function listFreeze()
{
	if(!$.checkDate("startDate","endDate")){
 	...
 	var form=$("#freezeFormSearch")[0];
	form.action="<%=request.getContextPath()%>/contract/freeze/freezeList.do?<%=WebConsts.FUNC_ID_KEY%>=03020501";
	App.submit(form);
}

//详情
function preFreeze(cntNum){
	var form=$("#freezeForm")[0];
	form.action="<%=request.getContextPath()%>/contract/freeze/viewFreeze.do?<%=WebConsts.FUNC_ID_KEY%>=0302050101&cntNum="+cntNum;
	App.submit(form);
}
...
<body>
<p:authFunc funcArray="0302050101"/>
<form action="" method="post" id="freezeFormSearch">
	<p:token/>
	<table>
		...
		<tr>
			<td colspan="4" class="tdWhite">
				<input type="button" value="查找" onclick="listFreeze();"/>
				<input type="button" value="重置" onclick="initEvent();"/>					
			</td>
		</tr>
	</table>
</form>
<form action="" method="post" id="freezeForm">
	<br/>	
	...
	<table id="listTab" class="tableList">
		...
		<c:forEach items="${cList}" var="c" varStatus="status">
			<tr onmouseover="setTrBgClass(this, 'trOnOver2');" onmouseout="setTrBgClass(this, 'trOther');">				
				...
				<td >
					<div class="update">
					    <a href="#" onclick="preFreeze('${c.cntNum}');" title="冻结"></a>
					</div>
				</td>	
			</tr>
		</c:forEach>
