...
function listDeliver()
{
	...
	form.action="<%=request.getContextPath()%>/contract/deliver/deliverList.do?<%=WebConsts.FUNC_ID_KEY%>=03020302";
	App.submit(form);
}
...
function preEdit(cntNum){
	...
	form.action="<%=request.getContextPath()%>/contract/deliver/deliverView.do?<%=WebConsts.FUNC_ID_KEY%>=0302030201&cntNum="+cntNum;
	App.submit(form);
}
	...
	if($("input[name='cntNums']:checked").size() < 1){
		...
		$("#auditDiv").dialog({
			...
			open:function()
			{
				...
				$('#deliverDutyCode').val('${deliverDutyCode}');
			},
			close:function(){
				$('#hideCntNums').empty();
				$( this ).dialog( "close" );
			...
			buttons: {
				"批量接收": function() {
					var url = "<%=request.getContextPath()%>/contract/deliver/accept.do?<%=WebConsts.FUNC_ID_KEY %>=0302030202";
				...
				"批量拒绝": function() {
					var url = "<%=request.getContextPath()%>/contract/deliver/reject.do?<%=WebConsts.FUNC_ID_KEY %>=0302030203";
					if(!App.valid("#auditForm")){return;}
					$('#auditForm').attr("action",url);
					App.submitShowProgress();//锁屏
					$('#auditForm').submit();
...
<form action="" method="post" id="deliverFormSearch">
	...
	<table>
		...
		<tr>
			...
			<td class="tdRight">
				<input type="text" id="cntNum" name="cntNum" value="${deliver.cntNum}"  class="base-input-text" maxlength="80" />
			</td>
			...
			<div class="ui-widget">
				<select id="cntType" name="cntType">
					...
					<forms:codeTable tableName="SYS_SELECT" selectColumn="PARAM_VALUE,PARAM_NAME"
					 ...
					 orderType="ASC"  selectedValue="${deliver.cntType}"/>
				</select>
			</div>
			</td>
		</tr>
		<tr>
			...
			<td class="tdRight">
				<input type="text" id="providerName" name="providerName" value="${deliver.providerName}" onclick="providerTypePage()" class="base-input-text" readonly/>
				<%-- <input type="hidden" id="providerCode" name="providerCode" value="${deliver.providerCode}"  class="base-input-text" />
			...
			<td class="tdRight" >
				<input type="text" id="startDate" name="startDate" valid maxlength='10' readonly="readonly" value="${deliver.startDate}" class="base-input-text" style="width:135px;"/>
				至
				<input type="text" id="endDate" name="endDate" valid maxlength='10' readonly="readonly" value="${deliver.endDate}" class="base-input-text" style="width:135px;"/>
			</td>	
		</tr>
		<tr>
			<td colspan="4" class="tdWhite">
				<input type="button" value="查找" onclick="listDeliver();"/>
				<input type="button" value="批量操作" onclick="deliverBatch()">
...
<form action="" method="post" id="deliverForm">
	...
	<table id="listTab" class="tableList">
		...
		<c:forEach items="${deliverList}" var="d" varStatus="status">
			<tr onmouseover="setTrBgClass(this, 'trOnOver2');" onmouseout="setTrBgClass(this, 'trOther');">	
				...
				<td >
					<div class="update">
					    <a href="#" onclick="preEdit('${d.cntNum}');" title="接受移交"></a>
					</div>
				</td>	
			</tr>
		</c:forEach>
		<c:if test="${empty deliverList}">
			<tr>
