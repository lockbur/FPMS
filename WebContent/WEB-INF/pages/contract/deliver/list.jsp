...
function listDeliver()
{
	...
	form.action="<%=request.getContextPath()%>/contract/deliver/list.do?<%=WebConsts.FUNC_ID_KEY%>=03020301";
	App.submit(form);
}
...
function preEdit(cntNum){
	...
	form.action="<%=request.getContextPath()%>/contract/deliver/view.do?<%=WebConsts.FUNC_ID_KEY%>=0302030101&cntNum="+cntNum;
	App.submit(form);
}
...
function deliverBatch(){
	if($("input[name='cntNums']:checked").size() < 1){
		App.notyError("请选择要移交的合同！");
		return false;
	}else{
		var url = "<%=request.getContextPath()%>/contract/deliver/deliver.do?<%=WebConsts.FUNC_ID_KEY %>=0302030102";
		$("#auditDiv").dialog({
			title:"发起移交",
			closeOnEscape:false,
			autoOpen: true,
			...
			buttons: {
				"确定": function() {
					...
					if(dutyTreeList.length < 1){
						App.notyError("移交机构不能为空，请选择移交机构！");
						return false;
					}
	...
	<table>
		...
		<tr>
			<td colspan="4" class="tdWhite">
				...
				<input type="button" value="批量移交" onclick="deliverBatch()">
	...
	<table id="listTab" class="tableList">
		...
		<c:forEach items="${dList}" var="d" varStatus="status">
			<tr onmouseover="setTrBgClass(this, 'trOnOver2');" onmouseout="setTrBgClass(this, 'trOther');">	
				...
				<td >
					<div class="update">
					    <a href="#" onclick="preEdit('${d.cntNum}');" title="发起移交"></a>
					</div>
				</td>	
			</tr>
		</c:forEach>
