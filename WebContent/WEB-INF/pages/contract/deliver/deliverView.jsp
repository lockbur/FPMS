...
function acceptDeliver(cntNum){
	...
	$("#auditDiv").dialog({
		...
		open:function()
		{
			$('#hideCntNums').html($("#cntNum"));
			$('#deliverDutyCode').val('${cnt.deliverDutyCode}');
		},
		buttons: {
			"确定": function() {
			...
			<td>
				...
				<tr class="collspan-control">
						<th colspan="4">
							...
							<input type="hidden" value="${cnt.cntNum}" id="cntNum" name="cntNums"/>
						</th>
					</tr>
					<tr>
...
<%-- 										<td><fmt:formatNumber type="number" value="${dzspItem.abcdeAmt}"/></td> --%>
						...
						<td class="tdLeft" colspan="4">
							<table>
								...
								<c:if test="${empty cnt.devices}">
									<tr>
										<td colspan="14" style="text-align: center;"><span class="red">没有找到相关信息！</span></td>
									</tr>
								</c:if>
							</table>
						</td>
		...
		<tr>
			<td colspan="4" >
				...
				<input type="button" value="接受" onclick="acceptDeliver('${cnt.cntNum}');"> 
				<input type="button" value="拒绝" onclick="reject('${cnt.cntNum}');"> 
