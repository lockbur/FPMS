...
function showDetail(payId,isPrePay,cntNum,monDataFlag,flag,isCreditNote,invoiceIdBlue){
	if(flag == 2){//预提待摊有回冲可以执行付款
		
	}else{
		if(monDataFlag == 0){//月结中
			App.notyError("正在月结中，不允许做付款确认。");
			return ;
		}
	}
	...
	$("#queryDetailForm #invoiceIdBlue1").val(invoiceIdBlue);
 	form.attr('action', '<%=request.getContextPath()%>/pay/paysure/detail.do?<%=WebConsts.FUNC_ID_KEY%>=03030602');
	App.submit(form);
}
function doValidate(){
	...
	<table id="condition">
		...
		<tr>
			<td colspan="4" class="tdWhite">
				<p:button funcId="03030601" value="查询"/> 
				...
				<tr onmouseover="setTrBgClass(this, 'trOnOver2');" onmouseout="setTrBgClass(this, 'trOther');">
					...
					<td>
						<c:if test="${sedList.isEnable=='Y'}">
							<div class="update">
								<a href="#" onclick="showDetail('${sedList.payId}','${sedList.isPrePay}','${sedList.cntNum}','${monDataFlag }','${flag }','${sedList.isCreditNote}','${sedList.invoiceIdBlue}')" title="确认"></a>
							</div>
						</c:if>
					</td>
				</tr>
