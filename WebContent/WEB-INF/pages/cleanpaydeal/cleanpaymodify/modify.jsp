...
// 	else if(!$.checkMoney($("#uncleanAmt").val()))
// 	{
// 		App.notyError("剩余结清金额格式有误！最多含两位小数的18位正浮点数。");
// 		$("#uncleanAmt").focus();
// 		return false;
// 	}
...
// 	var bercleanAmt = '${cleanedPayDetaiInfo.cleanAmt}';
// 	//判断剩余结清金额应小于实际剩余结清金额
// 	if(uncleanAmt < (leftAmt+parseFloat(bercleanAmt)))
// 	{
...
//已结清明细
function suspenseDetail(cleanPayId,sortId)
{
	var form=$("#paySuspenseDetailForm")[0];
	var url = "<%=request.getContextPath()%>/cleanpaydeal/cleanpay/queryCleanedDetail.do?<%=WebConsts.FUNC_ID_KEY%>=03050102&flag=3";
	url += "&normalPayId="+$("#normalPayId").val();
	url += "&sortId="+sortId;
...
function pageInit(){
	$_showWarnWhenOverLen1(document.getElementById("cleanReason"),200,'memoSpan1');
}
</script>
</head>
<body>
<p:authFunc funcArray="03050202,03050203"/>
...
<form action="" method="post" id="paySuspenseDealForm">
	...
	<table>
			...
			<td>
				<table>
					...
					<tr>
						...
						<td class="tdRight" width="25%">
							${payInfo.payId}
							<input type="hidden" id="normalPayId" name="normalPayId" value="${cleanPayBean.normalPayId}"/>
							<input type="hidden" id="sortId" name="sortId" value="${cleanPayBean.sortId}"/>
						</td>
					...
					<tr>
						...
						<td class="tdRight">
							<fmt:formatNumber type="number" value="${payInfo.cleanAmtIngTotal-cleanedPayDetaiInfo.cleanAmt}" minFractionDigits="2"/>
							<input type="hidden" id="cleanAmtIngTotal" value="${payInfo.cleanAmtIngTotal-cleanedPayDetaiInfo.cleanAmt}"/>
						</td>
					...
					<c:if test="${!empty payCleanedPayInfoList}">
						<c:forEach items="${payCleanedPayInfoList }" var="bean">
							<tr>
								...
								<td>
									<div class="detail">
										<a href="#" onclick="suspenseDetail('${bean.cleanPayId}','${bean.sortId}');" title="明细"></a>
									</div>
								</td>
							</tr>
						</c:forEach>
			...
			<td>
				<table>
					<tr>
						...
						<td class="tdRight">
							<input id="cleanAmt" name="cleanAmt" value="${cleanedPayDetaiInfo.cleanAmt }" maxlength="18" class="base-input-text" valid errorMsg="请输入结清金额" onkeyup="$.clearNoNum(this);" onblur="$.onBlurReplace(this);"/>
						</td>
					</tr>
					<tr>
						<td class="tdLeft">结清原因<span class="red">*</span><br/>(<span id='memoSpan1'>0/200</span>)</td>
						<td class="tdRight">
							<textarea id="cleanReason" cols="60" rows="5"  name="cleanReason" onkeyup="$_showWarnWhenOverLen1(this,200,'memoSpan1')" class="base-textArea" valid errorMsg="请输入结清原因">${cleanedPayDetaiInfo.cleanReason }</textArea>
						</td>
					</tr>
				</table>
			</td>
		...
		<tr>
			<td>
				<p:button funcId="03050202" value="保存"/>
				<%-- <p:button funcId="03050203" value="提交"/> --%>
