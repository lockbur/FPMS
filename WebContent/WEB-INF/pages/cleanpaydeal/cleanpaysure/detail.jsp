	...
	$("#auditDiv").dialog({
		...
		buttons: {
			"同意": function() {	
				...
				var form = $("#auditForm");
			 	form.attr('action', '<%=request.getContextPath()%>/cleanpaydeal/cleanpaysure/agree.do?<%=WebConsts.FUNC_ID_KEY%>=03050404');
				App.submit(form);
			},
			"退回":function(){
				if(!App.valid("#auditForm")){return;}
				var form = $("#auditForm");
			 	form.attr('action', '<%=request.getContextPath()%>/cleanpaydeal/cleanpaysure/back.do?<%=WebConsts.FUNC_ID_KEY%>=03050405');
				App.submit(form);
			},
...
function toAudit(button, url) {
	...
	$("#auditDiv").dialog( "option", "title", "暂收结清确认" ).dialog( "open" );
}
//已结清明细
function suspenseDetail(cleanPayId)
{
	var form=$("#paySuspenseDetailForm")[0];
	var url = "<%=request.getContextPath()%>/cleanpaydeal/cleanpay/queryCleanedDetail.do?<%=WebConsts.FUNC_ID_KEY%>=03050102&flag=5&cleanPayId="+cleanPayId;
...
<form action="" method="post" id="paySuspenseDetailForm"></form>
	...
	<table>
		...
		<tr>
			<td>
				<p:button funcId="03050403" value="暂收结清确认" onclick="toAudit"/>
