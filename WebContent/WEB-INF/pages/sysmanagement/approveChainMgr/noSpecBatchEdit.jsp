...
function selectPublicMatrs(){
	...
	App.ajaxSubmit("sysmanagement/approveChainMgr/selecstPublicMatrs.do?<%=WebConsts.FUNC_ID_KEY%>=0812010311",{data : data,async:false}, function(data) {
...
<body>
<p:authFunc funcArray="0812010312"/>
<form action="" method="post" id="reqtypeForm">
<input type="hidden" name="aprvType" id="aprvType" value="${selectInfo.aprvType }"/>
<input type="hidden" name="dataYear" id="dataYear" value="${selectInfo.dataYear }"/>
<p:token/>
	<table>
		<tr>
			<th colspan="2">审批链批量维护
		...
		<tr>
			<td>
				<table id="approveChainTable">
					<tr>
						<td colspan="2">
							<c:forEach items="${dutyList}" var="org2" varStatus="i">
								<div   style="width:33%;float: left; margin-left: 0px;padding: 0px; text-align: left;">
									<input type="hidden" id="tool${i.index }_hide" value="${org2.noUpCount }"/>
									<label>
										<input type="checkbox" style="margin-left: 0px;" class="duty" value="${org2.dutyCode }" onclick="selectPublicMatrs()" name="dutys" />
									<span class="dutyToolTip" id="tool${i.index }">(${org2.dutyCode }-<span  style="color:red">${org2.noUpCount }</span>)</span>
									<forms:StringTag length="20" value="${org2.dutyName}"/>
									</label>
								</div>
							</c:forEach>
						</td>
				...
				<table id="tjTable">
					<tr>
						...
						<td class="tdRight">
							...
							<forms:OrgSelPlugin   suffix="001" triggerEle="#tjTable tr matrAuditDeptName,matrAuditDept::name,id" rootNodeId="${userOrg1Code}" 
					...
					<tr>
						<td colspan="4" align="center">
							<p:button funcId="0812010312" value="提交"/>
