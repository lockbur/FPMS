...
function checkCglCode(){
	...
	var url = "sysmanagement/matrtype/checkCglType.do?<%=WebConsts.FUNC_ID_KEY%>=01080705";
...
function addSubmit(){
	if(check()){
		if(confirm('确认新增？')){
			...
			form.action = "<%=request.getContextPath()%>/sysmanagement/matrtype/addPrepaidCode.do?<%=WebConsts.FUNC_ID_KEY%>=01080702";
			App.submit(form);
		}
	}	
}
...
function check(){
	...
	if($.trim($('#cglCode').val())==''||$.trim($('#cglCode').val())==null)
	{
		App.notyError("费用核算码不能为空");
		return false;
	}
	if (cglCode.length!=4) {
		App.notyError("费用核算码长度不为4，请检查！");
		return false;
	}
	if (checkCglCode()) {
		App.notyError("费用核算码已存在，请重新输入！");
		return false;
	}
...
function showLongPrepaidCodeFee(){
	...
	if ($.trim(longPrepaidCode) != '' ) {
		if($("#fc_select").find("td:eq(1)").attr("colspan")!='1' ){
			...
			$("#fc_select").append("<td class='tdLeft' width='25%'>短期待摊核算码</td><td class='tdRight' width='25%' colspan='1'><input type='text' onkeyup='onlyNum(this);' name='shortPrepaidCode' id='shortPrepaidCode' maxlength='4' class='base-input-text'></td>");
			$("#fc_select").append("<td class='tdLeft' width='25%'>长期待摊固定对应的费用核算码</td><td class='tdRight' width='25%'><input type='text' onkeyup='onlyNum(this);' name='longPrepaidCodeFee' id='longPrepaidCodeFee' maxlength='4' class='base-input-text'></td>");
		}	
...
<body>
	...
	<p:authFunc funcArray="01080702" />
		<table>
			<tr>
				<th colspan="4">核算码新增</th>
			</tr>
			<tr>
				<td class="tdLeft" width="25%"><span class="red">*</span>费用核算码</td>
				<td class="tdRight" width="25%" >
					<input type="text" id="cglCode" name="cglCode" onkeyup="onlyNum(this);" class="base-input-text" maxlength="4"/>
				</td>
				<td class="tdLeft" width="25%">长期待摊核算码</td>
				<td class="tdRight" width="25%">
					<input type="text" id="longPrepaidCode" name="longPrepaidCode" onkeyup="onlyNum(this);" class="base-input-text" maxlength="4" onblur="showLongPrepaidCodeFee();"/>
				</td>
			</tr>
			<tr id="fc_select">
