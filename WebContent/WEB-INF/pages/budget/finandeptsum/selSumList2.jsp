...
function view(tmpltId, matrCode ,matrAuditDept){
	...
	var url ="<%=request.getContextPath()%>/budget/finandeptsum/secondView.do?<%=WebConsts.FUNC_ID_KEY %>=02040202";
	var form =$("#viewForm");
	form.attr("action",url);
	App.submit(form);
	
}
function sub(tmpltId,matrAuditDept,matrCode){
	$("#matrCode").val(matrCode);
	var afterAmt =$("#auditAmt").val();
	$("#afterAmt").val(afterAmt);
	$("#auditAmt2").val(afterAmt);
	var beforeAmt = $("#auditAmtHidden").val();
	$("#beforeAmt").val(beforeAmt);
	$("#matrAuditDept").val(matrAuditDept);
	if(!$.checkMoney(afterAmt))
	{
	...
	var url ="<%=request.getContextPath()%>/budget/finandeptsum/secondSubmit.do?<%=WebConsts.FUNC_ID_KEY %>=02040203";
...
function download(tmpltId,matrAuditDept,matrCode){
	var url="<%=request.getContextPath()%>/budget/finandeptsum/download.do?<%=WebConsts.FUNC_ID_KEY %>=02040204";
}
</script>
</head>
<body>
<p:authFunc funcArray="02040201,02040202,02040204"/>
<form action="" id="viewForm" method="post">
		...
		<input type="hidden" name="afterAmt" id="afterAmt"/>
		...
		<p:token/>
			...
			<tr>
				...
				<tr>
					<td colspan="4">
						<!-- 查找指定部门的汇总情况 -->
						<p:button funcId="02040201" value="查询"/>
		...
		<table id="listTab" class="tableList">
			...
			<c:forEach items="${finanSumList}" var="finanSum" varStatus="status">
				<tr onmouseover="setTrBgClass(this, 'trOnOver2');" onmouseout="setTrBgClass(this, 'trOther');">
				    ...
				    <td><input type="hidden" id="auditAmtHidden" value="${finanSum.auditAmt}"> 
				    	<input type="text" id="auditAmt" value="${finanSum.auditAmt}"/>
				    </td>
				    <td >${finanSum.auditFlagName}</td>
				    <td>
						<input type="button" value="提交" onclick="sub('${finanDeptSum.tmpltId}','${finanSum.matrAuditDept}','${finanSum.matrCode}')"/>
						<p:button funcId="02040204"  value="下载" />
				    </td>
				</tr>
			</c:forEach>
