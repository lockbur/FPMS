...
function getResolve(tmpltId , montCode){
	var form = $("#resolveQueryForm")[0];
	var url = "<%= request.getContextPath() %>/budget/budgetResolve/resolveQuery/getResolveQuery.do?<%=WebConsts.FUNC_ID_KEY %>=02060201&tmpltId="+tmpltId+"&montCode="+montCode;
	form.action = url;
	form.submit();
	form.action = "";
}
...
<body>
<p:authFunc funcArray="020602"/>
<form action="" method="post" id="resolveQueryForm">
	<table>
		...
		<tr>
			<td colspan="4" class="tdWhite">
				<p:button funcId="020602" value="查询"/>
	...
	<table class="tableList" id="resolveTable">
		...
		<c:forEach items="${budgetResolveList}" var="resolve">
			<tr onmouseover="setTrBgClass(this, 'trOnOver2');" onmouseout="setTrBgClass(this, 'trWhite');" class="trWhite">
				...
				<td>${resolve.replyAmt}</td>
				<td>${resolve.splitedAmt}</td>
				<td>
					<div class="detail"><a href="#" onclick="getResolve('${resolve.tmpltId}','${resolve.montCode}');" title="查看详情"></a></div>
				</td>
			</tr>
		</c:forEach>
