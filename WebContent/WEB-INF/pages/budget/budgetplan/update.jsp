...
<script type="text/javascript">
	...
	function pageInit(){
		...
		//页面加载时进行文件描述文字个数统计
		$_showWarnWhenOverLen1($("#memo")[0],100,'fileDescLogSpan');
	}
    ...
    //修改预算模板
    function saveUpdate(){
		var orgRelIds = getOrgIdsByTb();
		if(orgRelIds.length == 0){
			alert("预算模板至少选中一个可用的责任中心！");
			return;
		}
    	$('#availableOrgList').val(orgRelIds);
//     	alert($('#availableOrgList').val());
    	var form = $("#budgetPlanDetailForm")[0];
    	form.action="<%=request.getContextPath()%>/budget/budgetplan/update.do?<%=WebConsts.FUNC_ID_KEY %>=02010301";
    	App.submit(form);
    }
...
<%-- 				<forms:OrgSelPlugin suffix="1" radioFlag="false" selHalfFlag="false" dynamicUpdateFlag="true"  --%>
<%-- 									jsVarName="treePlugin1" rootNodeId="${budgetInfo.org21Code}" leafOnlyFlag="true" --%>
<%-- 									initValue="${budgetInfo.availableOrgList}" changeFun="updateZRZX" dialogFlag="true" buttonName="选择可用机构"/> --%>
		...
		<tr>
			...
			<td class="tdRight" width="25%" colspan="3" style="height: 60px">
				<textarea rows="4" cols="100" id="memo" name="memo" class="base-textArea" onkeyup="$_showWarnWhenOverLen1(this,100,'fileDescLogSpan')">${budgetInfo.memo}</textarea>
			</td>
		</tr>
	</table>
	<P></P>
	...
	<P></P>
	<input type="button" value="确定修改" onclick="saveUpdate()">
</form>
</body>
</html>