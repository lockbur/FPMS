...
<%-- 							<input type="text" id="suspenseAmt2" value="${payInfo.suspenseAmt}" name="suspenseAmt" value="0" maxlength="18" class="base-input-text"  onkeyup="setTwoValue(this,'suspenseAmt1','请输入暂收金额');" onblur="showInfo(this);" valid errorMsg="请输入暂收金额" /> --%>
						...
						<c:if test="${payInfo.suspenseAmt == 0}">
							<td class="tdLeft"></td>
							<td class="tdRight">
							</td>
						</c:if>
...
<%-- 							<textarea id="invoiceMemo" readonly="readonly" cols="60" rows="5"  name="invoiceMemo" class="base-textArea">${payInfo.invoiceMemo}</textArea> --%>
				...
				<table id="payMon" class="tableList">
					...
					<tr>
						...
						<th width="8%">设备型号</th>
						<th width="10%">总金额(元)</th>
						<th width="10%">已付金额(元)</th>
						<th width="10%">冻结金额(元)</th>
						<th width="10%">发票分配金额(元)</th>
						<th width="11%">发票行说明</th>
					</tr>
					...
					<c:forEach items="${payDevices}" var="bean">
						<tr onmouseover="setTrBgClass(this, 'trOnOver2');" onmouseout="setTrBgClass(this, 'trWhite');">
							...
							<td>
								...
								<c:if test="${bean.execAmt != bean.payedAmt }">
<%-- 									<input id="payInvAmts" name="payInvAmts" value="${bean.subInvoiceAmt }" maxlength="18" style="width: 100px;"  class="base-input-text" /> --%>
									${bean.subInvoiceAmt }
								</c:if>
							</td>
							<td>
<%-- 								<input id="payIvrowMemos" name="payIvrowMemos" class="base-input-text" style="width: 100px;" maxlength="250" value="${bean.ivrowMemo }" /> --%>
								${bean.ivrowMemo }
							</td>
						</tr>
					</c:forEach>
...
<%-- 		    	<p:button funcId="03030403" value="提交" onclick="submitPay"/> --%>
