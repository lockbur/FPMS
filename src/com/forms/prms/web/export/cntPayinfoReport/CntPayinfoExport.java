package com.forms.prms.web.export.cntPayinfoReport;
...
public class CntPayinfoExport implements IExportDataDeal{

	CntPayinfoReportService es = CntPayinfoReportService.getInstance();
	CntPayinfoReportService excelDealService = CntPayinfoReportService.getInstance(); // 用于更新导出任务结果状态和导出备注
	...
	public void getSimpleExcelData(String taskId, Map params, SimplifyBatchExcelExporter excelExporter)
		...
		CntPayinfoReportBean bean = new CntPayinfoReportBean();

		bean.setOrgFlag((String)params.get("orgFlag"));
		bean.setCntType((String)params.get("cntType"));
		bean.setFeeType((String)params.get("feeType"));
		bean.setProjName((String)params.get("projName"));
		bean.setMatrType((String)params.get("matrType"));
		bean.setCglCode((String)params.get("cglCode"));
		bean.setFeeStartDate((String)params.get("feeStartDate"));
		...
		// //插入excel数据内容
		try {
			List<CntPayinfoReportBean> cList = new ArrayList<CntPayinfoReportBean>();
			cList = es.exportExcute(paramMap);
			beansMap.put("0", cList);
			exportSucFlag = true;
			exportMemo = "处理成功，可下载";
			//增加一条汇总数据
		} catch (Exception e) {
