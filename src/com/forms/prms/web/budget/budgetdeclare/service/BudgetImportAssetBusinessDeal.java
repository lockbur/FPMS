...
import com.forms.prms.web.budget.budgetdeclare.domain.AssetBudgetDetailBean;
 ...
 */
public class BudgetImportAssetBusinessDeal implements IBusinessDeal{
	...
	public void end(String batchNo, String arg1, Map beans, ExcelImportInfo importInfo) throws Exception {
		...
		ExcelBean<AssetBudgetDetailBean> bean = (ExcelBean<AssetBudgetDetailBean>)beans.get("bean");

		//2.遍历具体的Excel内部Bean信息,并写到数据库
		List<AssetBudgetDetailBean> excelInfoList = bean.getTemplateInfo();
		...
		int i=1;
		for(AssetBudgetDetailBean dbean : excelInfoList){
			dbean.setRowSeq(""+i++);
		}
		budget.getAssetList().addAll(excelInfoList);
		
		
		
		budgetService.insertAssetBudgetDetail(budget);
